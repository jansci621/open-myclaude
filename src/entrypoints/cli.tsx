...
// MACRO polyfill - must be imported before main.tsx
...
async function main(): Promise<void> {
  ...
  // Fast-path for `claude webchat` / `claude web`: start web-based chat interface
  if (args[0] === 'webchat' || args[0] === 'web') {
    profileCheckpoint('cli_webchat_path');
    const { webchatMain } = await import('./webchat.js');
    await webchatMain(args.slice(1));
    return;
  }
