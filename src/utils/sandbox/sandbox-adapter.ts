...
// Optional imports for internal package
...
// Create a stub sandbox manager that provides safe defaults
const createStubSandboxManager = () => ({
  checkDependencies: () => ({ errors: ['Sandbox runtime not available'], warnings: [] }),
  isSupportedPlatform: () => false,
  wrapWithSandbox: (_config: any, callback: any) => callback,
  initialize: async () => {},
  updateConfig: () => {},
  reset: async () => {},
  getFsReadConfig: () => null,
  getFsWriteConfig: () => null,
  ...
  getProxyPort: () => null,
  getSocksProxyPort: () => null,
  getLinuxHttpSocketPath: () => null,
  getLinuxSocksSocketPath: () => null,
  waitForNetworkInitialization: async () => {},
  getSandboxViolationStore: () => null,
  annotateStderrWithSandboxFailures: () => {},
  cleanupAfterCommand: async () => {},
})

let BaseSandboxManager: any = createStubSandboxManager()
let SandboxRuntimeConfigSchema: any = null
let SandboxViolationStore: any = null

try {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
  const sandbox = require('@anthropic-ai/sandbox-runtime')
  if (sandbox.SandboxManager) {
    BaseSandboxManager = sandbox.SandboxManager
  }
  SandboxRuntimeConfigSchema = sandbox.SandboxRuntimeConfigSchema
  SandboxViolationStore = sandbox.SandboxViolationStore
} catch {
  // Package not available - sandbox features will be disabled
}
